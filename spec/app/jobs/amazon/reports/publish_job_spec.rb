describe Amazon::Reports::PublishJob do
  context :perform do
    ...
    Sidekiq::Testing.inline! do
      When do
        VCR.use_cassette("app/jobs/amazon/reports/publish_job/custom_transactions", match_requests_on: %i[body method]) do
          ::Amazon::Reports::Ingest::CustomTransactions::IngestJob.new.perform user_id, "#{prefix}#{custom_transactions}", false
        end
      end
      When do
        VCR.use_cassette("app/jobs/amazon/reports/publish_job/sponsored_products", match_requests_on: %i[body method]) do
          ::Amazon::Reports::Ingest::SponsoredProducts::IngestJob.new.perform user_id, "#{prefix}#{sponsored_products}", false
        end
      end
      When { ::Amazon::Reports::PublishJob.new.perform user_id }
