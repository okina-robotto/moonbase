class CreateAmazonSponsoredProductsAllTime < ActiveRecord::Migration[6.0]
  def change
    execute <<-SQL
      CREATE MATERIALIZED VIEW amazon_sponsored_products_all_time AS
        ...
        GROUP BY user_id, date_trunc('month', date_time), portfolio_name, currency_exchange_rate_id, campaign_name, ad_group_name, sku, asin
        ORDER BY date_trunc('month', date_time), currency_exchange_rate_id ASC;
    SQL
  end
end
